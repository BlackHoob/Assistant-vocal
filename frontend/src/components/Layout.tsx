              ...
              <div className={`absolute left-0 bottom-full mb-1 w-80 max-w-[calc(100vw-2rem)] rounded-2xl shadow-xl border overflow-hidden z-50 ${
                ...
                <div className={`flex items-center justify-between px-4 py-3 border-b ${dark ? 'border-gray-800' : 'border-gray-100'}`}>
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-semibold ${dark ? 'text-gray-200' : 'text-gray-800'}`}>{t('notif_title')}</span>
...
<div className={`border-t pt-4 mt-2 ${dark ? 'border-gray-800' : 'border-gray-50'}`}>
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${dark ? 'bg-gray-950' : 'bg-white'}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Page content */}
        <main className={`flex-1 overflow-hidden flex flex-col transition-colors ${dark ? 'bg-gray-950' : 'bg-white'}`}>
          <Outlet />
        </main>
